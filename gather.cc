...
Gather::Bookmark* Gather::set_bookmark(int size) {
...
void Gather::write_bookmark(Bookmark* bmark, const void* ptr) {
    char* pc = (char *) ptr;
...
int Gather::write(const void* p, int n) {
    assert(chunk_.empty() || !chunk_.front()->fully_read());
...
int Gather::peek(void* p, int n) const {
    assert(chunk_.empty() || !chunk_.front()->fully_read());
...
int Gather::read_from_Gather(Gather& m, int n) {
    assert(chunk_.empty() || !chunk_.front()->fully_read());
...
int Gather::read_from_fd(int fd) {
    assert(chunk_.empty() || !chunk_.front()->fully_read());
...
bool Gather::content_size_gt(int size) const {
